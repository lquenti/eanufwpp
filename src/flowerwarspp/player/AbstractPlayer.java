...
/**
 * Abstrakte Basis-Klasse welche die grundlegende Implementation eines Spielers beschreibt, welcher
 * die Anforderungen des Interfaces {@link Player} erfüllt. Die einzige abstrakte Methode deren
 * Implementation gefordert wird, ist {@link #requestMove()}. Diese Methode fordert einen Zug vom
 * jeweiligen Spieler an, und leitet diesen Zug an die Methode {@link #request()} weiter.
 */
abstract class AbstractPlayer implements flowerwarspp.preset.Player {
	 ...
	 * noch nicht initialisiert worden ist.
	 */
	private static final String noInitMessage = "Der Spieler muss zuerst initialisiert werden!";

	/**
	 * Eine vordefinierte Nachricht einer {@link Exception}, welche geworfen wird, wenn eine der
	 * Methoden {@link #confirm(Status)}, {@link #request()} oder {@link #update(Move, Status)} zu
	 * einem unerwarteten Zeitpunkt aufgerufen worden wird.
	 */
	private static final String unexpectedCallMessage = "Unerwarteter Methoden-Aufruf.";

	/**
	 * Eine vordefinierte Nachricht einer {@link Exception}, welche geworfen wird, wenn {@link
	 * #request()} zum falschen Zeitpunkt aufgerufen worden ist.
	 */
	...
	/**
	 * Eine vordefinierte Nachricht einer {@link Exception}, welche geworfen wird, wenn {@link
	 * #update(Move, Status)} zum falschen Zeitpunkt aufgerufen worden ist.
	 */
	...
	/**
	 * Eine vordefinierte Nachricht einer {@link Exception}, welche geworfen wird, wenn {@link
	 * #confirm(Status)} zum falschen Zeitpunkt aufgerufen worden ist.
	 */
	private static final String cycleConfirmMessage =
			unexpectedCallMessage + " Es haette request() aufgerufen werden sollen.";

	/**
	 * Eine vordefinierte Nachricht einer {@link Exception}, welche geworfen wird, wenn es eine
	 * Disparität zwischen den Status des Hauptprogramms und des eigenen Spielbretts gab.
	 */
	private static final String statusErrorMessage =
			"Der Status des Hauptprogramms und der Status des Spielbretts dieses Spielers stimmen " +
					"nicht ueberein!";
	/**
	...
	/**
	 * Ermöglicht den Zugriff auf relevante Daten des Spielbretts, welche für die Verifikation und
	 * die Ausarbeitung von Spielzügen benötigt werden.
	 */
	...
	/**
	 * Wird genutzt, um den aktuellen Status des Spieler-Lebenszyklus (also der Zyklus, welcher im
	 * Interface {@link flowerwarspp.preset.Player} diktiert wird) darzustellen.
	 */
	private PlayerFunction cycleState;

	/**
	 * Ein <code>default</code>-Konstruktor, welcher die Instanzvariablen mit Basiswerten
	 * initialisiert.
	 */
	...
	public final Move request() throws Exception, RemoteException {
		...
		log(DEBUG, "move of type " + move.getType() + " returned from player through request(): " +
				move);
	...
	/**
	 * Wrapper für die {@link Log#log(LogLevel, LogModule, String)}-Methode, welcher das {@link
	 * LogModule} setzt und den Spieler anhand seiner Farbe identifiziert.
	 *
	 * @param level
	...
	/**
	 * Fordert einen Zug vom Spieler an, wie der {@link Move} angefordert wird, wird der jeweiligen
	 * Implementation der abstrakten Klasse überlassen.
	 *
	 * @return Der vom Spieler zurückgegebene Zug.
	 *
	 * @throws Exception
	 * 		Falls der jeweilige Spieler keinen Zug angeben konnte.
	 */
	 ...
	 * @param status
	 * 		Status des Spielbretts des Hauptprogramms nach Ausführen des Zuges des Gegenspielers
	 *
	 * @throws Exception
	...
	/**
	 * Initialisiert einen Spieler, indem dieser mit einem neuen Spielbrett passender Größe und der
	 * gewünschten Farbe versehen wird. Falls diese Methode waärend eines laufenden Spiels
	 * aufgerufen wird, wird dieses beendet und mit dem neu initialisierten Spieler wird ein neues
	 * Spiel begonnen.
	 *
	 * @param boardSize
	 * 		Spielbrettgröße
	 ...
	 *
	 * @throws Exception
	 * 		Falls während der Initialisierung ein Fehler auftrat
	 * @throws RemoteException
	 * 		falls bei der Netzwerkkommunikation etwas schief gelaufen ist
	 */
	@Override
	public void init(int boardSize, PlayerColor playerColor) throws Exception, RemoteException {
		...
		// Instanzvariable der Spielerfarben setzen.
		...
		log(INFO, "Initialized new player with color " + playerColor + " on a board with size " +
				boardSize);
	}
	 ...
	 *
	 * @param board
	 * 		Das neue zu verwendene Spielbrett.
	 */
	...
	/**
	 * Ein unterstützender enum um die Ausführung der durch das Interface {@link
	 * flowerwarspp.preset.Player} verlangten Methoden in der korrekten Reihenfolge zu sichern.
	 *
